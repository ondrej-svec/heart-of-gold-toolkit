...
Shared skills own conversational meaning and workflow policy. The Pi extension owns explicit launch commands, optional decision presentation and existing work guardrails, not interpretation of the assistant's prose. Source 0.2.5 contains the accepted native question-style interaction and its aligned RPC adapter; release verification/publication is tracked separately. The corrected immutable local preview `0.2.5-preview-fc565ca7a034` is now globally pinned and fresh-process verified after exact source-only authorization; the subsequent approval and ordinary-question tests in this session received human acceptance. This is acceptance of the native preview, not cross-harness release readiness. The [preview activation record](../reviews/2026-09-14-hog-ask-025-preview-activation.md) identifies the verified bytes and unchanged settings/resource composition. The previous 0.2.4 ordinary Enter-to-Back UX was rejected after reload; publication of that archive remains stopped despite restored npm login. The [correction plan](../plans/2026-09-14-fix-hog-ask-question-ux-plan.md) records the completed native acceptance gate; RPC/guidance alignment is implemented in source, while clean-release verification, exact artifact approval and publication remain separate gates. See the [rollout record](../plans/2026-09-14-chore-hog-ask-024-rollout-plan.md) for exact scope and status. The [native proof record](../reviews/2026-09-13-hog-ask-native-proof.md) distinguishes source evidence from physical-terminal and live usability claims.
...
**RPC uses the same direct-answer semantics through standard select/input**, not custom TUI or `editor`. A preset submits after one select response; valid custom input submits without another screen. An optional **Answer with a note** action enables a note route before choosing/writing an answer; the note input identifies that answer and sends both on valid submit. Blank notes are allowed, invalid custom/note drafts remain visible for correction, and the user can exit note mode before choosing an answer. No ordinary review screen is shown.

Only approvals expose exact-scope review: the named Send action is first, Back/change is explicit, and selecting an approval intent never submits by itself. Request changes opens required feedback; valid, invalid and whitespace feedback drafts require explicit clearing before bare approval is offered again. Every dialog retains question/scope/recommendation context; display labels map explicitly to IDs. In Pi 0.85.1, select/input accept AbortSignal but editor does not. Cancellation of any RPC dialog dismisses the interaction without opening another dialog. RPC clients own keyboard/layout and repeat handling; this is not a native Editor/caret/Back or physical-input guarantee. Print/JSON and missing UI return `unavailable` without asking the model to guess.
