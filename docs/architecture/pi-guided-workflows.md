...
The native editor-area card distinguishes focus, draft selection, review and final **Send**. Real alternatives are separate from the persistent inline Editor, which is reachable by arrows, Tab/Shift-Tab, typing/paste and whole-row clicks. Back retains text/caret. Expanded paste text, not display markers, is validated and submitted. Decisions retain a separate optional note editor. Approval presents **Approve as written / Request changes / Not now**: Request changes requires reviewed free text, and pending feedback must be explicitly cleared before bare approval. Approval has no generic note field; the core remains conservative for any legacy/custom adapter that supplies one.

Approval review accepts a subsequent deliberate confirmation after the opening activation's release (Enter by default). Without observed release provenance it shows the explicitly accepted **Tab → Enter** fallback, never inferring a physical release from a timer or Pi's broad Kitty-active boolean. Back/refocus clears both guards; reported activation/Tab repeats do not submit or arm. Raw legacy streams cannot prove physical key releases: the fallback requires a separate Tab action and rejects Enter-only repetition. Ordinary decisions retain neutral Back review and Ctrl+Enter Send (or Tab through note to Send); Ctrl+Enter in an editor only reviews. Escape goes back, while the host clear-editor binding (Ctrl+C by default) dismisses even invalid drafts. Hints show the first configured binding, not every alias.

Long content wraps with fixed controls. Approval-review arrows scroll directly; entry and decision-review arrows also scroll at focus edges because fullscreen owns PageUp/Down for the transcript. Fullscreen mouse regions come from rendered cells; hover does not take typing focus, native Editor caret/drag behavior is preserved, and continuing/reflowed gestures cannot activate a relocated control. Completed mouse submission does not reclaim focus from Pi's restored editor. Regular mode retains terminal-owned mouse/scrollback rather than promising unsupported pointer controls.
