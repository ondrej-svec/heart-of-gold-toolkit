...
**hog_ask** follows Pi's bundled `question` design: compact numbered choices and Enter to submit ordinary answers directly. Tab adds an optional note before sending; custom text and notes retain their drafts/caret on Back. **Approve as written / Request changes / Not now** use exact-scope review: Enter opens review, the next Enter confirms, and Esc goes back—no Tab unlock. Reported activation repeats are ignored; raw legacy Enter cannot distinguish a held key from a new press. Focus changes restart approval review without clearing feedback. Feedback never silently becomes consent. Print/JSON returns `unavailable`; no answer executes an action automatically. RPC uses standard select/input dialogs: ordinary selections and custom text submit directly; choose **Answer with a note** first to compose a note for the displayed answer before sending. Approval alone has a review screen, with Send first and Back explicit. RPC clients own keyboard/layout handling, and cancelling any RPC dialog dismisses the question. The [correction plan](docs/plans/2026-09-14-fix-hog-ask-question-ux-plan.md) records native acceptance and release verification.

Publication and local activation are separate: source commits and `/reload` alone do not replace a pinned immutable release. The locally activated 0.2.4 decision UX was rejected after reload; publication of that archive was stopped. See the [0.2.4 rollout record](docs/plans/2026-09-14-chore-hog-ask-024-rollout-plan.md), [interaction architecture](docs/architecture/pi-guided-workflows.md), [native implementation proof](docs/reviews/2026-09-13-hog-ask-native-proof.md), and [historical 0.2.3 proof](docs/reviews/2026-09-13-hog-ask-proof.md).
