...
A learning-first, offline guide inside Heart of Gold. **Read explanations, not run actions.** Shell, finding, tmux, Neovim, writing decisions and personal tools share one small catalog.
...
The direct runtime needs **Node 22+**, no dependencies, Bun, Pi, credentials or network. In a terminal, optional fzf selects an explanation; otherwise a numbered prompt works. Piped output never waits. `--plain` avoids all presentation tools. `--glow` explicitly enables isolated Markdown formatting (not an external pager).

- Enter reads; Esc quits fzf. In a card, Enter or `q` returns; Ctrl-C quits.
...
Tests use synthetic profiles and temporary homes. Optional host tests require an already-installed Bun; packaging tests require already-installed npm. Neither installs anything. For the opt-in real-terminal smoke on macOS, inspect `tests/terminal-smoke.py --help`; it uses Python's standard library, already-installed fzf/Glow and an explicitly reviewed tldr C 1.6.1 binary, with disposable homes and OS-denied network access. Node 24/25 macOS are locally exercised; Node 22 and Linux are CI targets, not claimed local results.
